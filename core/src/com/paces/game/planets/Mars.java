...
public class Mars extends Planetas {

    float zPlaneta = 0f;

    public Mars(float escala, float x, float y, float z, float luzDirX, float luzDirY, float luzDirZ){

        animacionInfo = new AnimacionInfo(xInfo, yInfo, wInfo, hInfo, 10);
        info = Main.assets.manager.get("Planetas_Info/Marte.png");
        this.zPlaneta = z;
        eje = 25f;
        batch = new SpriteBatch();
        setSrings();

        textura = Main.assets.manager.get("TexturasPlanetas/marte.jpg");
